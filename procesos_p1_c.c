...
int main(int argc, char *argv[]) {
    printf("PROGRAMA 1: CREACION: Proceso padre creado con PID: %d, instante de ción: %ld\n", getpid(), time(NULL));
    ...
    pid_t pid1 = fork();
    if (pid1 < 0) {
        perror("PROGRAMA 1: CREACION: Error al r el 1er proceso hijo");
        exit(1);
    } else if (pid1 == 0) {
        printf("PROGRAMA 1: CREACION: 1er proceso hijo do con PID: %d, PID del padre: %d\n", getpid(), getppid());
        ...
        for (i = 0; i < vecesSincronizacion; i++) {
            ...
            printf("PROGRAMA 1: COMUNICACION: 1er proceso hijo escribe: %d\n", data->number);
            ...
            printf("PROGRAMA 1: COMUNICACION: 1er proceso hijo lee respuesta: %d\n", data->response);
        }
        ...
        pid_t pid2 = fork();
        if (pid2 < 0) {
            perror("PROGRAMA 1: CREACION: Error al r el 2do proceso hijo");
            exit(1);
        } else if (pid2 == 0) {
            printf("PROGRAMA 1: CREACION: 2do proceso hijo do con PID: %d, PID del padre: %d\n", getpid(), getppid());
            ...
            for (i = 0; i < vecesSincronizacion; i++) {
                ...
                printf("PROGRAMA 1: COMUNICACION: 2do proceso hijo lee: %d\n", data->number);
                data->response = data->number * 2;
                printf("PROGRAMA 1: COMUNICACION: 2do proceso hijo escribe respuesta: %d\n", data->response);

...
int main(int argc, char *argv[]) {
    ...
    if (pid1 < 0) {
        ...
        for (i = 0; i < vecesSincronizacion; i++) {
            ...
            printf("COMUNICACION: ITERACIÓN %d: PROGRAMA 2: 1er proceso hijo con PID: %d ha esperado: %ld 2dos\n", i, getpid(), time(NULL) - t);
        }
        ...
        if (pid2 < 0) {
            ...
            for (i = 0; i < vecesSincronizacion; i++) {
                ...
                printf("COMUNICACION: ITERACIÓN %d: PROGRAMA 2: 2do proceso hijo con PID: %d va a esperar: %d 2dos\n", i, getpid(), wait_time);

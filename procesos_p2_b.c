...
int main(int argc, char *argv[]) {
    ...
    printf("CREACION: PROGRAMA 2: Proceso padre creado con PID: %d, instante de ción: %ld\n", getpid(), time(NULL));
    ...
    if (pid1 < 0) {
        ...
        for (i = 0; i < vecesSincronizacion; i++) {
            ...
            printf("COMUNICACION: PROGRAMA 2: 1er proceso hijo con PID: %d ha esperado: %ld 2dos en la iteración: %d\n", getpid(), time(NULL) - t, i);
        }
        ...
        if (pid2 < 0) {
            ...
            for (i = 0; i < vecesSincronizacion; i++) {
                ...
                printf("COMUNICACION: PROGRAMA 2: 2do proceso hijo con PID: %d va a esperar: %d 2dos en la iteración: %d\n", getpid(), wait_time, i);

...
int main(int argc, char *argv[]) {
    ...
    pid_t pid1 = fork();
    if (pid1 < 0) {
        perror("CREACION: PROGRAMA 3: Error al r el 1er proceso hijo");
        exit(1);
    } else if (pid1 == 0) {
        printf("CREACION: PROGRAMA 3: 1er proceso hijo creado con PID: %d, PID del padre: %d\n", getpid(), getppid());
        ...
        for (i = 0; i < vecesSincronizacion; i++) {
            ...
            printf("COMUNICACION: ITERACION %d: PROGRAMA 3: 1er proceso hijo escribe: %d\n", i, data->number);
            ...
            printf("COMUNICACION: ITERACION %d: PROGRAMA 3: 1er proceso hijo lee respuesta: %d\n", i, data->response);
        }
        ...
        pid_t pid2 = fork();
        if (pid2 < 0) {
            perror("CREACION: PROGRAMA 3: Error al r el 2do proceso hijo");
            exit(1);
        } else if (pid2 == 0) {
            printf("CREACION: PROGRAMA 3: 2do proceso hijo creado con PID: %d, PID del padre: %d\n", getpid(), getppid());
            ...
            for (i = 0; i < vecesSincronizacion; i++) {
                ...
                printf("COMUNICACION: ITERACION %d: PROGRAMA 3: 2do proceso hijo lee: %d\n", i, data->number);
                data->response = data->number * 2;
                printf("COMUNICACION: ITERACION %d: PROGRAMA 3: 2do proceso hijo escribe respuesta: %d\n", i, data->response);

...
int main(int argc, char *argv[]) {
    ...
    if (pid1 < 0) {
        ...
        for (i = 0; i < vecesSincronizacion; i++) {
            ...
            printf("PROGRAMA 1: 1er proceso hijo con PID: %d ha esperado: %ld 2dos en la iteración: %d\n", getpid(), time(NULL) - t, i);
        }
        ...
        if (pid2 < 0) {
            ...
            for (i = 0; i < vecesSincronizacion; i++) {
                ...
                printf("PROGRAMA 1: 2do proceso hijo con PID: %d va a esperar: %d 2dos en la iteración: %d\n", getpid(), wait_time, i);
